  ...
  useEffect(() => {
    const fetchSetlists = async () => {
      try {
        ...
        // Filter entries where `Song` matches the current song name
        const filteredSets = data.filter((entry) => entry.Song === song.Name);
        setSets(filteredSets);
        console.log("Filtered setlists:", filteredSets);
      } catch (err) {
